...
- **경로 시각화 영상 생성**: GPS 좌표를 기반으로 전체 이동 경로와 실시간 위치를 보여주는 400x400 비율의 애니메이션 영상(`.mp4`)을 생성함.
- **데이터 보정**: 
...
운동 경로를 애니메이션 영상으로 제작함. 상단에 시간과 필터링된 속도가 오버레이됨.
```powershell
python generate_video.py
```
- **입력**: `Zepp20260407070319.fit`
- **출력**: `Zepp20260407070319route.mp4`
- **시각화 규칙**:
  - 회색 선: 전체 경로 (배경)
  - 오렌지색 선: 현재까지 이동한 경로
  - 빨간 점: 현재 위치
